...
class ReadAligner {
	...
	public static void main(String[] args) throws IOException
	{
		...
		while(true){
			...
			try {
				reads = ShortRead.readFastqReads(readFile1, 1000);
			...
			// building hashmap for reads - XXOO
			...
			// now doing alignment for forward strand hashmaps
			...
			//			// (I do 10k at a time; adjust this size as needed).
			//			try {
			//				reads = ShortRead.readFastqReads(readFile2, 100);
			//
			//				lim++;
			//				if (reads.length == 0)
			//					break;
			//
			//				System.out.println("Reading for rev strand" + reads.length + " reads");
			//			} catch (IOException e) {
			//				System.out.println("Failed to read short reads");
			//				return;
			//			}
			...
			//ShortRead[] reads = ShortRead.readAllFastqReads(args[1]);
			for(int i = 0; i < reads.length; i++){
				if(alignment.containsKey(reads[i].sequence())){
					...
					// printing the output
					if(unique){
						strContent = reads[i].name() + "    " + Math.abs(uniqueRef[0]) + "    " + sign[0] + "    " + qualityScore + '\n';
					}
					else{
						strContent = reads[i].name() + " !! " + Math.abs(uniqueRef[0]) + "    " + sign[0] + "    " + Math.abs(uniqueRef[1]) + "    " + sign[1]+ '\n' ;
					}
