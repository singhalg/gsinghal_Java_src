...
public class TreeMap<K extends Comparable<K>,V> implements Map<K,V> {
	 ...
	 * if it finds the node associated with the key, it calls the helpCompare() method on the node, which in turn, calls the successor() method
	 * successor method return a new retNode to the helpCompare() method, which can replace the node to be deleted
	 * successor() also deletes the original copy of the root element of retNode
	 * the node returned by helpCompare() replaces the node to be deleted
	 */
